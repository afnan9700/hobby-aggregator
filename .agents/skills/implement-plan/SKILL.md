...
2. Relevant convention docs in `.agents/docs/conventions/` if needed.
3. Codebase files named in the plan or required by direct dependencies.

## Process
1. Read the planned step(s).
2. Implement only the requested step(s).
3. Keep changes aligned with the plan.
4. Verify the result immediately after implementation (where verification is required).
